...
def having_Sub_Domain(url):
    parsed = tldextract.extract(url)
    # number of subdomain parts (0 means none)
    return len(parsed.subdomain.split('.')) if parsed.subdomain else 0

def SSLfinal_State(url):
    # Try to open an SSL socket and validate certificate common name (simple)
    parsed = tldextract.extract(url)
...
def Favicon(url):
    ...
    if not soup:
        return 0
    # check if favicon is from same domain or external
    link = soup.find("link", rel=lambda v: v and 'icon' in v.lower())
    if not link or not link.get("href"):
        return 0
    href = link["href"]
    # if href contains domain different from page domain -> suspicious (1)
...
def port(url):
    # if port is specified in URL and is unusual -> suspicious
    m = re.search(r":(\d+)", url)
    if m:
        p = int(m.group(1))
        # return port number or flag; models often use boolean
        return p
    return 80 if url.startswith("http://") else 443

def HTTPS_token(url):
    # presence of 'https' token in domain portion
    parsed = url.split("://")[-1]
    host = parsed.split("/")[0]
    return 1 if "https" in host.lower() else 0

def Request_URL(url):
    # Count img/script/css src that point to external domains -> ratio
    r, soup = _get_soup(url)
    if not soup:
        return 0.0
    resources = []
...
def SFH(url):
    # server form handler: check <form action=""> attribute
    r, soup = _get_soup(url)
    if not soup:
        return 0
    ...
    for f in forms:
        ...
        if a == "" or a == None:
            suspicious += 1
        elif tldextract.extract(a).registered_domain and tldextract.extract(a).registered_domain != page_domain:
            suspicious += 1
    return suspicious
...
def Abnormal_URL(url):
    # heuristic: domain not matching WHOIS registrar or domain empty
    parsed = tldextract.extract(url)
    domain = parsed.registered_domain
    if not domain:
        return 1
...
def on_mouseover(url):
    ...
    if not soup:
        return 0
    # search for onmouseover attributes or inline JS that changes status bar
...
def RightClick(url):
    ...
    if not soup:
        return 0
    # search for contextmenu prevent default in scripts or oncontextmenu attribute
    if soup.find(attrs={"oncontextmenu": True}):
        return 1
    # look for JS that disables right-click (simple text match)
...
# Placeholders (need external APIs or web scraping)
def web_traffic(url):
    # returns numeric rank or -1 if unknown
    # You can use Alexa (deprecated), SimilarWeb API, or Tranco list
    return -1

def Page_Rank(url):
    # historic PageRank not public; place holder
    return -1

def Google_Index(url):
    # you would call Google Search Console API or do a site: query (not ideal)
    return -1

def Links_pointing_to_page(url):
    # needs backlink API (e.g., Majestic, Ahrefs, Moz) or Google Search Console
    return -1

def Statistical_report(url):
    # optional aggregate! Could be a score (sum of suspicious flags)
    # return simple count of suspicious flags as example
...
def extract_all_features(url):
    return {
        ...
        # predicted_column is left for model output
    }
